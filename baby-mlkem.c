 ...
 * st[21..24]; rebuild rate words 0..20 only when a refill is needed. */
...
/* Keep the matrix state live through all three initial XOF rate blocks. */
static MLKEM_NOINLINE __attribute__((optimize("no-schedule-insns2"))) void
keccakf8_sparse_matrix_3_store_blocks(
  ...
  __m512i ga = zero, ge = zero, gi = zero, go = zero, gu = zero;
  __m512i ka = zero, ke = zero, ki = zero, ko = zero, ku = zero;
  __m512i ma = zero, me = zero, mi = zero, mo = zero, mu = zero;
  __m512i sa = _mm512_set1_epi64((long long)(0x80ULL << 56));
  __m512i se = zero, si = zero, so = zero, su = zero;
  ...
  for (int block = 0; block < 3; block++) {
    ...
    size_t block_off = (size_t)block * 168;
    MLKEM_KECCAKF8_STORE_RATE4(stream, block_off, 0, ba, be, bi, bo);
    MLKEM_KECCAKF8_STORE_RATE4(stream, block_off, 32, bu, ga, ge, gi);
    MLKEM_KECCAKF8_STORE_RATE4(stream, block_off, 64, go, gu, ka, ke);
    MLKEM_KECCAKF8_STORE_RATE4(stream, block_off, 96, ki, ko, ku, ma);
...
static void ntt_inv_mont_before_final_avx512(poly256 f) {
  ...
  for (int block = 0; block < 8; block++) {
    ...
    x = ntt_inv_mont_level_i16x32_avx512(
        x, partner, MLKEM_INV_MONT_DENSE_LO_AVX512(0, block),
        MLKEM_INV_MONT_DENSE_HI_AVX512(0, block),
        (__mmask32)0xccccccccu);

    partner = _mm512_permutexvar_epi64(swap_qword_pairs, x);
    x = ntt_inv_mont_level_i16x32_avx512(
        x, partner, MLKEM_INV_MONT_DENSE_LO_AVX512(1, block),
        MLKEM_INV_MONT_DENSE_HI_AVX512(1, block),
        (__mmask32)0xf0f0f0f0u);

    partner = _mm512_permutexvar_epi64(swap_qword_quads, x);
    x = ntt_inv_mont_level_i16x32_avx512(
        x, partner, MLKEM_INV_MONT_DENSE_LO_AVX512(2, block),
        MLKEM_INV_MONT_DENSE_HI_AVX512(2, block),
  ...
  for (int level = 4, length = 32; level < 6; level++, length <<= 1) {
    ...
    for (int start = 0; start < N; start += 2 * length, i++) {
      __m512i zeta_lo = MLKEM_INV_MONT_SCALAR_LO_AVX512(level, i);
      __m512i zeta_hi = MLKEM_INV_MONT_SCALAR_HI_AVX512(level, i);
...
ntt_inv_mont_before_final4_tail_level_clang_avx512(
    poly256 f0, poly256 f1, poly256 f2, poly256 f3,
    int length, int root_offset, int reduce_sum) {
...
/* Reuse each inverse-twiddle load across the three u outputs and v. */
static void ntt_inv_mont_before_final4_shared_avx512(
    poly256 f0, poly256 f1, poly256 f2, poly256 f3) {
  const __m512i swap_qword_pairs =
      _mm512_setr_epi64(1, 0, 3, 2, 5, 4, 7, 6);
  const __m512i swap_qword_quads =
...
#define MLKEM_INV_SHARED_LEVEL2 ntt_inv_mont_level_i16x32_avx512
#define MLKEM_INV_SHARED_LEVEL3 ntt_inv_mont_level_lazy_i16x32_avx512
  /* Canonical inputs allow reductions only at levels 2 and 5. */
#endif
  for (int block = 0; block < 8; block++) {
    ...
    __m512i zeta_lo = MLKEM_INV_MONT_DENSE_LO_AVX512(0, block);
    __m512i zeta_hi = MLKEM_INV_MONT_DENSE_HI_AVX512(0, block);
    ...
    zeta_lo = MLKEM_INV_MONT_DENSE_LO_AVX512(1, block);
    zeta_hi = MLKEM_INV_MONT_DENSE_HI_AVX512(1, block);
    ...
    zeta_lo = MLKEM_INV_MONT_DENSE_LO_AVX512(2, block);
    zeta_hi = MLKEM_INV_MONT_DENSE_HI_AVX512(2, block);
    ...
    partner = _mm512_permutexvar_epi64(swap_qword_quads, x1);
    x1 = MLKEM_INV_SHARED_LEVEL2(
        x1, partner, zeta_lo, zeta_hi, (__mmask32)0xff00ff00u);
    partner = _mm512_permutexvar_epi64(swap_qword_quads, x2);
    x2 = MLKEM_INV_SHARED_LEVEL2(
        x2, partner, zeta_lo, zeta_hi, (__mmask32)0xff00ff00u);
    partner = _mm512_permutexvar_epi64(swap_qword_quads, x3);
    x3 = MLKEM_INV_SHARED_LEVEL2(
        x3, partner, zeta_lo, zeta_hi, (__mmask32)0xff00ff00u);

    zeta_lo = MLKEM_INV_MONT_DENSE_LO_AVX512(3, block);
    zeta_hi = MLKEM_INV_MONT_DENSE_HI_AVX512(3, block);
    ...
    _mm512_storeu_si512((void *)(f2 + 32 * block), x2);
    _mm512_storeu_si512((void *)(f3 + 32 * block), x3);
  }
...
#endif
#undef MLKEM_INV_SHARED_LEVEL3
#undef MLKEM_INV_SHARED_LEVEL2
#undef MLKEM_INV_SHARED_LEVEL1
#undef MLKEM_INV_SHARED_LEVEL0
}

static MLKEM_ALWAYS_INLINE void ntt_inv_add_mont_final_chunk_avx512(
  ...
  ntt_inv_mont_scale_pair_i16x32_avx512(a, b, &scaled0, &scaled1);
  scaled0 = ntt_add_mod_q_i16x32_avx512(
      scaled0, _mm512_loadu_si512((const void *)add_lo));
  scaled1 = ntt_add_mod_q_i16x32_avx512(
      scaled1, _mm512_loadu_si512((const void *)add_hi));
  _mm512_storeu_si512((void *)out_lo, scaled0);
  _mm512_storeu_si512((void *)out_hi, scaled1);
}

/* Keep the wide four-polynomial schedule out of prepared encryption. */
static MLKEM_NOINLINE void
ntt_inv_add4_mont_final_shared_avx512(
    const poly256 add0, const poly256 add1, const poly256 add2,
    const poly256 add3, poly256 out0, poly256 out1, poly256 out2,
    poly256 out3) {
  ntt_inv_mont_before_final4_shared_avx512(out0, out1, out2, out3);
  for (int j = 0; j < N / 2; j += 32) {
    ntt_inv_add_mont_final_chunk_avx512(
...
static void ntt_tail_before_l1_mont_lazy_raw_avx2(poly256 f) {
  ...
  for (int start = 0, i = 0; start < N; start += 32, i++) {
    ...
    __m256i a = load_i16x4_quad_avx2(
        f + start + 24, f + start + 16, f + start + 8, f + start);
    __m256i b = load_i16x4_quad_avx2(f + start + 28, f + start + 20,
                                     f + start + 12, f + start + 4);
#else
    __m256i a = load_i16x4_quad_avx2(
...
#if defined(MLKEM_AVX2_REUSE_INV_FORWARD_TAIL)
    store_i16x4_quad_avx2(f + start + 24, f + start + 16, f + start + 8,
                          f + start, _mm256_add_epi16(a, t));
    store_i16x4_quad_avx2(f + start + 28, f + start + 20, f + start + 12,
                          f + start + 4, _mm256_sub_epi16(a, t));
#else
...
static void ntt_tail_mont_lazy_raw_avx2(poly256 f) {
  ...
  for (int start = 0, i = 0; start < N; start += 32, i++) {
    ...
    __m256i a = load_i16x2_oct_avx2(
        f + start + 28, f + start + 24, f + start + 20, f + start + 16,
        f + start + 12, f + start + 8, f + start + 4, f + start);
    __m256i b = load_i16x2_oct_avx2(
        f + start + 30, f + start + 26, f + start + 22, f + start + 18,
        f + start + 14, f + start + 10, f + start + 6, f + start + 2);
#else
    __m256i a = load_i16x2_oct_avx2(
...
static MLKEM_ALWAYS_INLINE void
ntt_tail_mont_lazy_raw_avx512_impl(poly256 f, int shared_table) {
  const __m512i swap_qword_pairs =
      _mm512_setr_epi64(1, 0, 3, 2, 5, 4, 7, 6);
  const __m512i swap_qword_quads =
      _mm512_setr_epi64(2, 3, 0, 1, 6, 7, 4, 5);
...
#define MLKEM_NTT_TAIL_MONT_HI_SELECT(level, index) \
  ...
  for (int block = 0; block < 8; block++) {
    ...
    x = ntt_forward_level_i16x32_avx512(
        x, partner, MLKEM_NTT_TAIL_MONT_LO_SELECT(0, block),
        MLKEM_NTT_TAIL_MONT_HI_SELECT(0, block),
        (__mmask32)0xff00ff00u);

    partner = _mm512_permutexvar_epi64(swap_qword_pairs, x);
    x = ntt_forward_level_i16x32_avx512(
        x, partner, MLKEM_NTT_TAIL_MONT_LO_SELECT(1, block),
        MLKEM_NTT_TAIL_MONT_HI_SELECT(1, block),
        (__mmask32)0xf0f0f0f0u);

    partner = _mm512_rol_epi64(x, 32);
    x = ntt_forward_level_i16x32_avx512(
        x, partner, MLKEM_NTT_TAIL_MONT_LO_SELECT(2, block),
        MLKEM_NTT_TAIL_MONT_HI_SELECT(2, block),
        (__mmask32)0xccccccccu);
    _mm512_storeu_si512((void *)(f + 32 * block), x);
  }

#undef MLKEM_NTT_TAIL_MONT_LO_SELECT
...
/* Congruent lazy output is in [-440,4570] over the full K=3 bound. */
static MLKEM_ALWAYS_INLINE __m512i
ntt_acc4_madd_reduce_lazy_i32x16(__m512i x) {
  const __m512i reciprocal = _mm512_set1_epi32(20159);
  const __m512i q = _mm512_set1_epi32(Q);
  __m512i quot = _mm512_srai_epi32(
      _mm512_mullo_epi32(_mm512_srai_epi32(x, 10), reciprocal), 16);
  return _mm512_sub_epi32(x, _mm512_mullo_epi32(quot, q));
}
...
#pragma clang loop unroll(disable)
  for (int i = 0; i < K; i++) {
    ...
    ntt_tail_mont_lazy_raw_avx512_impl(f[i], 1);
  }
}
...
/* Share each precomputed gamma vector across the three keygen factors. */
static MLKEM_ALWAYS_INLINE void
ntt_acc4_asym_c0_factors3_mont_clang_avx512(
  ...
  *y2_c0 = ntt_acc4_asym_c0_factor_mont_clang_avx512(
      y2, gamma_lo, gamma_hi);
}
#else
static MLKEM_ALWAYS_INLINE __m512i
ntt_acc4_asym_c0_factor_mont_gcc_avx512(
    __m512i y, __m512i gamma_lo, __m512i gamma_hi) {
  __m512i weighted =
      ntt_mont_mul_precomp_i16x32_avx512(y, gamma_lo, gamma_hi);
  return _mm512_mask_mov_epi16(
...
ntt_acc4_asym_c0_factors3_mont_gcc_avx512(
  ...
  __m512i gamma_hi = _mm512_slli_epi32(
      _mm512_cvtepu16_epi32(_mm256_loadu_si256(
          (const __m256i *)(const void *)(
              GAMMA_MONT_HI_GCC_AVX512 + pair))),
      16);
  /* QINV recovers gamma_lo because Q * QINV == 1 mod 2^16. */
  const __m512i qinv = _mm512_set1_epi16((int16_t)-3327);
  __m512i gamma_lo = _mm512_mullo_epi16(gamma_hi, qinv);
  *y0_c0 = ntt_acc4_asym_c0_factor_mont_gcc_avx512(
      y0, gamma_lo, gamma_hi);
  *y1_c0 = ntt_acc4_asym_c0_factor_mont_gcc_avx512(
      y1, gamma_lo, gamma_hi);
  *y2_c0 = ntt_acc4_asym_c0_factor_mont_gcc_avx512(
      y2, gamma_lo, gamma_hi);
}
#endif
...
/* Return one canonical accumulation tile without materializing it. */
static MLKEM_ALWAYS_INLINE __m512i ntt_acc4_asym_madd_value_avx512(
    const poly256 a0, const poly256 a1, const poly256 a2, int offset,
    __m512i y0_c0, __m512i y1_c0, __m512i y2_c0,
...
static MLKEM_ALWAYS_INLINE void ntt_acc4_asym_lazy_block_avx512(
  ...
  __m256i c0_16 = _mm512_cvtepi32_epi16(c0);
  __m256i c1_16 = _mm512_cvtepi32_epi16(c1);
  __m256i lo = _mm256_unpacklo_epi16(c0_16, c1_16);
  __m256i hi = _mm256_unpackhi_epi16(c0_16, c1_16);
  __m512i packed = _mm512_inserti64x4(
      _mm512_castsi256_si512(lo), hi, 1);
...
/* Reuse gamma-weighted secret factors across all A^T columns. */
static MLKEM_ALWAYS_INLINE void
...
static MLKEM_ALWAYS_INLINE __m512i ntt_add_canonical_i16x32_avx512(
    __m512i a, __m512i b) {
  const __m512i q = _mm512_set1_epi16(Q);
  const __m512i q_minus_1 = _mm512_set1_epi16(Q - 1);
  __m512i sum = _mm512_add_epi16(a, b);
  __mmask32 ge_q = _mm512_cmpgt_epi16_mask(sum, q_minus_1);
  return _mm512_mask_sub_epi16(sum, ge_q, sum, q);
}

static MLKEM_ALWAYS_INLINE void
byte_encode_d12_i16x32_register_avx512(__m512i v, uint8_t *out) {
  byte_encode_d12_i16x16_register_avx512(
      _mm512_castsi512_si256(v), out);
  byte_encode_d12_i16x16_register_avx512(
      _mm512_extracti64x4_epi64(v, 1), out + 24);
}
...
/* Keep both keygen encodings and the final public coefficients in-register. */
static MLKEM_ALWAYS_INLINE void
ntt_mul_acc3_cols3_fused_final_encode_add_madd512_avx512(
    const poly256 ahat[K][K], const poly256 b[K], const poly256 ehat[K],
    poly256 out[K], uint8_t encoded_b[K * 384],
  ...
  for (int offset = 0, pair = 0; offset < N;
    ...
    byte_encode_d12_i16x32_register_avx512(
        y1, encoded_b + 384 + encoded_offset);
    byte_encode_d12_i16x32_register_avx512(
        y2, encoded_b + 768 + encoded_offset);
    ...
    c0 = ntt_add_canonical_i16x32_avx512(
        c0, _mm512_loadu_si512((const void *)(ehat[0] + offset)));
    c1 = ntt_add_canonical_i16x32_avx512(
        c1, _mm512_loadu_si512((const void *)(ehat[1] + offset)));
    c2 = ntt_add_canonical_i16x32_avx512(
        c2, _mm512_loadu_si512((const void *)(ehat[2] + offset)));
    _mm512_storeu_si512((void *)(out[0] + offset), c0);
    _mm512_storeu_si512((void *)(out[1] + offset), c1);
    _mm512_storeu_si512((void *)(out[2] + offset), c2);
    byte_encode_d12_i16x32_register_avx512(
        c0, encoded_out + encoded_offset);
    byte_encode_d12_i16x32_register_avx512(
        c1, encoded_out + 384 + encoded_offset);
    byte_encode_d12_i16x32_register_avx512(
        c2, encoded_out + 768 + encoded_offset);
  }
}
...
static inline void sample_poly_cbd_eta2_store2_signed_avx2(
    __m128i bytes, int16_t *out0, int16_t *out1) {
  const __m128i lut = _mm_setr_epi8(0, 1, 1, 2, -1, 0, 0, 1,
                                   -1, 0, 0, 1, -2, -1, -1, 0);
  const __m128i mask = _mm_set1_epi8(0x0f);
  __m128i lo8 = _mm_shuffle_epi8(lut, _mm_and_si128(bytes, mask));
...
static void mlkem_prf_cbd_eta2x6_32(const uint8_t seed[32],
  ...
  keccakf8(st);
  sample_poly_cbd_eta2x6_signed_state_avx512(st, out0, out1, out2, out3,
                                      out4, out5);
}
...
mlkem_encrypt_prf_cbd_eta2_32_sample_tail_clang_avx512(
  ...
  for (int word = 0; word < 25; word++) {
    st[word] = _mm512_setzero_si512();
  }
  st[0] = _mm512_set_epi64(
      (long long)load64_le(rho + 0), (long long)load64_le(seed + 0),
      (long long)load64_le(seed + 0), (long long)load64_le(seed + 0),
      (long long)load64_le(seed + 0), (long long)load64_le(seed + 0),
      (long long)load64_le(seed + 0), (long long)load64_le(seed + 0));
  st[1] = _mm512_set_epi64(
      (long long)load64_le(rho + 8), (long long)load64_le(seed + 8),
      (long long)load64_le(seed + 8), (long long)load64_le(seed + 8),
      (long long)load64_le(seed + 8), (long long)load64_le(seed + 8),
      (long long)load64_le(seed + 8), (long long)load64_le(seed + 8));
  st[2] = _mm512_set_epi64(
      (long long)load64_le(rho + 16), (long long)load64_le(seed + 16),
      (long long)load64_le(seed + 16), (long long)load64_le(seed + 16),
      (long long)load64_le(seed + 16), (long long)load64_le(seed + 16),
      (long long)load64_le(seed + 16), (long long)load64_le(seed + 16));
  st[3] = _mm512_set_epi64(
      (long long)load64_le(rho + 24), (long long)load64_le(seed + 24),
      (long long)load64_le(seed + 24), (long long)load64_le(seed + 24),
      (long long)load64_le(seed + 24), (long long)load64_le(seed + 24),
      (long long)load64_le(seed + 24), (long long)load64_le(seed + 24));
  st[4] = _mm512_set_epi64(
      0x1f0202LL, 0x1f06LL, 0x1f05LL, 0x1f04LL,
      0x1f03LL, 0x1f02LL, 0x1f01LL, 0x1f00LL);
  st[16] = _mm512_set_epi64(
      0, (long long)(0x80ULL << 56), (long long)(0x80ULL << 56),
      (long long)(0x80ULL << 56), (long long)(0x80ULL << 56),
      (long long)(0x80ULL << 56), (long long)(0x80ULL << 56),
      (long long)(0x80ULL << 56));
  st[20] = _mm512_set_epi64(
      (long long)(0x80ULL << 56), 0, 0, 0, 0, 0, 0, 0);

  keccakf8(st);
  for (int word = 0; word < 25; word++) {
    __m128i hi = _mm256_extracti128_si256(sample_ntt8_hi256(st[word]), 1);
    uint64_t value = (uint64_t)_mm_cvtsi128_si64(_mm_srli_si128(hi, 8));
    tail_st[word] = value;
  }
  ...
  sample_ntt_parse_init_avx2();
  int count = sample_ntt_parse_stream_avx2_ready(
      (const uint8_t *)tail_st, 21 * sizeof(*tail_st), tail, 0);
  while (count < N) {
    keccakf(tail_st);
    count = sample_ntt_parse_stream_avx2_ready(
        (const uint8_t *)tail_st, 21 * sizeof(*tail_st), tail, count);
  }
}
#endif
...
mlkem_keygen_prf_cbd_eta2_32_sample_tail_avx512(
  ...
  for (int lane = 0; lane < 25; lane++) {
    tail_st[lane] = sample_ntt8_lane6_u64(st[lane]);
  }
...
static MLKEM_ALWAYS_INLINE void sha3_tail_set_noise3_avx2(
    __m256i st[25], const uint8_t seed[32], uint8_t nonce0,
    uint8_t nonce1, uint8_t nonce2) {
  const __m256i keep_tail_lane = _mm256_set_epi64x(0, 0, -1LL, 0);

  for (int word = 0; word < 25; word++) {
    st[word] = _mm256_and_si256(st[word], keep_tail_lane);
  }
  for (int word = 0; word < 4; word++) {
  ...
  st[4] = _mm256_or_si256(
      st[4], _mm256_set_epi64x(
                 (long long)((uint64_t)nonce2 | (0x1fULL << 8)),
                 (long long)((uint64_t)nonce1 | (0x1fULL << 8)), 0,
                 (long long)((uint64_t)nonce0 | (0x1fULL << 8))));
...
static MLKEM_ALWAYS_INLINE void mlkem_prf_cbd_eta2_i8_scalar_32(
    const uint8_t seed[32], uint8_t nonce, int8_t out[N]) {
  uint64_t st[25] = {0};
  st[0] = load64_le(seed + 0);
  st[1] = load64_le(seed + 8);
  st[2] = load64_le(seed + 16);
  ...
  for (int block = 0; block < 8; block++) {
    __m128i bytes = _mm_loadu_si128(
        (const __m128i *)(const void *)(st + 2 * block));
    sample_poly_cbd_eta2_store2_i8_avx2(
        bytes, out + 32 * block, out + 32 * block + 16);
  }
}

static MLKEM_NOINLINE void mlkem_decaps_keccakf4_avx2(__m256i st[25]) {
  keccakf4(st);
}

/* Hash first, then fill the tail continuation's six idle x4 lanes with
 * dependent encryption noise. Only nonce 6 remains for scalar Keccak. */
static MLKEM_NOINLINE void sha3_512_sample_ntt_tail_noise6_gcc_avx512(
    const uint8_t in0[32], const uint8_t in1[32], const uint8_t rho[32],
    poly256 tail, uint8_t ghash[64], poly256 r0, poly256 r1, poly256 r2,
    int8_t e10[N], int8_t e11[N], int8_t e12[N], int8_t e2[N]) {
  __m256i st[25];
  uint64_t stream[63];

  for (int word = 0; word < 25; word++) st[word] = _mm256_setzero_si256();
  st[0] = _mm256_set_epi64x(0, 0, (long long)load64_le(rho + 0),
                            (long long)load64_le(in0 + 0));
  st[1] = _mm256_set_epi64x(0, 0, (long long)load64_le(rho + 8),
                            (long long)load64_le(in0 + 8));
  ...
  st[6] = _mm256_set_epi64x(0, 0, 0, (long long)load64_le(in1 + 16));
  st[7] = _mm256_set_epi64x(0, 0, 0, (long long)load64_le(in1 + 24));
  st[8] = _mm256_set_epi64x(0, 0, 0,
                            (long long)0x8000000000000006ULL);
  st[20] = _mm256_set_epi64x(0, 0, (long long)(0x80ULL << 56), 0);

  mlkem_decaps_keccakf4_avx2(st);
  for (int word = 0; word < 8; word++) {
    uint64_t value = keccak_lane0_u64(st[word]);
    memcpy(ghash + 8 * word, &value, sizeof(value));
  }
  for (int word = 0; word < 21; word++) stream[word] = keccak_lane1_u64(st[word]);
  ...
  sha3_tail_set_noise3_avx2(st, ghash + 32, 3, 4, 5);
  mlkem_decaps_keccakf4_avx2(st);
  for (int word = 0; word < 16; word++) {
    sample_poly_cbd_eta2_store1_i8_avx2(_mm256_castsi256_si128(st[word]),
                                        e10 + 16 * word);
    sample_poly_cbd_eta2_store2_i8_avx2(_mm256_extracti128_si256(st[word], 1),
                                        e11 + 16 * word, e12 + 16 * word);
  }
  for (int word = 0; word < 21; word++) stream[42 + word] = keccak_lane1_u64(st[word]);
  mlkem_prf_cbd_eta2_i8_scalar_32(ghash + 32, 6, e2);
  ...
  while (count < N) {
    uint64_t extra[21];
    mlkem_decaps_keccakf4_avx2(st);
    for (int word = 0; word < 21; word++) extra[word] = keccak_lane1_u64(st[word]);
    count = sample_ntt_parse_stream_avx2_ready(
        (const uint8_t *)extra, sizeof(extra), tail, count);
  }
}
...
#if defined(BABY_MLKEM_DISABLE_INTERNAL_CACHES)
#define MLKEM_INTERNAL_CACHES_ENABLED 0
#else
static int mlkem_internal_caches_enabled = 1;
#define MLKEM_INTERNAL_CACHES_ENABLED mlkem_internal_caches_enabled
#endif
...
#if defined(__clang__)
  ntt_mul_acc3_cols3_fused_final_encode_add_madd512_avx512(
      kpke_public_cache_ahat, shat, ehat, kpke_public_cache_that, dk_pke,
      ek_pke);
#else
  ntt_mul_acc3_cols3_fused_final_encode_madd512_avx512(
...
static inline void kpke_encrypt_prepared_public(const uint8_t *m, size_t mlen,
  ...
  (void)eta2_i8_prepared;
        ...
        defined(__AVX512BW__) && defined(__clang__))
...
#if defined(__AVX2__) && !(defined(__AVX512F__) && defined(__AVX512BW__))
  ntt_mul_acc3_avx2(kpke_secret_cache_shat[0], u[0],
  ...
  */
  ...
  if (!MLKEM_INTERNAL_CACHES_ENABLED) {
    ...
    sha3_512_sample_ntt_tail_noise6_gcc_avx512(
        mdash, h, rho, kpke_public_cache_ahat[2][2], ghash,
        rhat[0], rhat[1], rhat[2], e1_i8[0], e1_i8[1], e1_i8[2], e2_i8);
    public_prepared = 1;
    eta2_i8_prepared = 1;
#elif defined(__AVX2__) && !defined(__AVX512F__)
    uint8_t inbuf[64];
