...
PIN_CPU="${PIN_CPU:-0}"
REPORT_FILE="${REPORT_FILE:-/tmp/baby-mlkem-goal-${PROFILE}-speed.txt}"
...
if [ "$PRINT_PROFILE" != "0" ] && [ "$PRINT_PROFILE" != "1" ]; then
  echo "PRINT_PROFILE must be 0 or 1" >&2
  exit 2
fi
...
if [ "$PRINT_PROFILE" = "1" ]; then
  goal_speed_print_profile
  exit 0
fi
for required_command in git rg sha256sum lscpu nm objdump; do
...
BUILD_TAG="$(printf '%s-%s' "$C_COMPILER" "$PROFILE" | tr '/ ' '__')"
...
if [ "$PROFILE" = "avx2" ]; then
  if objdump -d "$ROOT_DIR/baby_mlkem768_product.o" | rg -n '%zmm|%k[0-7]'; then
    echo "AVX512 register use detected in AVX2-only production artifact" >&2
    exit 2
  fi
  if nm "$ROOT_DIR/baby_mlkem768_product.o" | rg -n 'avx512'; then
    echo "AVX512 symbol detected in AVX2-only production artifact" >&2
    exit 2
  fi
  local_avx512_instruction_audit=pass
else
  local_avx512_instruction_audit=not-applicable
fi
...
LOCAL_BENCH_REUSE=1 \
SKIP_LOCAL_BUILD=1 \
LOCAL_BENCH_BIN="$ROOT_DIR/bench_productc" \
CLEAN_LOCAL_BUILD_ARTIFACTS=0 \
LOCAL_ROUNDTRIP_METRIC=mlkem_roundtrip_core_ns_per_op \
GIT_REPOS_PREUPDATED=1 \
PIN_CPU="$PIN_CPU" \
C_COMPILER="$C_COMPILER" \
UPDATE_REPOS=1 \
ALLOW_CACHED_COMPARATOR=0 \
BENCH_ISA_PROFILE="$BENCH_ISA_PROFILE" \
BENCH_PROFILE_TAG="$BENCH_PROFILE_TAG" \
KYBER_DIR="$KYBER_DIR" \
...
: > "$revision_report"
printf "local_avx512_instruction_audit=%s\n" \
  "$local_avx512_instruction_audit" >> "$revision_report"
