...
goal_speed_configure_profile() {
  ...
  _goal_speed_export_common "$compiler"
  case "$profile" in
    native)
      ...
      MLKEM_NATIVE_HARNESS_CFLAGS="-O3 $cflags -maes -fomit-frame-pointer -std=c99"
      ...
      BORINGSSL_CXX_FLAGS="-O3 -march=native"
      ...
      BOTAN_CXXFLAGS="-O3 $cflags -fomit-frame-pointer -fno-semantic-interposition"
    ...
    avx2)
      ...
      BOTAN_HARNESS_CXXFLAGS="-O3 $cflags"
      OPENSSL_CFLAGS="-O3 -fno-semantic-interposition $cflags"
  ...
  export MLKEM_NATIVE_HARNESS_CFLAGS UPSTREAM_CFLAGS FAIR_UPSTREAM_CFLAGS
...
goal_speed_print_profile() {
  local key
  for key in \
    BENCH_ISA_PROFILE BENCH_PROFILE_TAG OPT_CFLAGS EXTRA_CFLAGS ARCH_CFLAGS \
    ASFLAGS MLKEM_NATIVE_AUTO MLKEM_NATIVE_HARNESS_CFLAGS UPSTREAM_CFLAGS \
    ...
    LIBJADE_HARNESS_CFLAGS BOTAN_CXXFLAGS BOTAN_DISABLED_MODULES \
