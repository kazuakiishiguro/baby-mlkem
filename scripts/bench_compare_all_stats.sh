...
printf "mlkem_native_cflags=%s\n" "${MLKEM_NATIVE_CFLAGS:--march=native -mavx2 -mbmi2 -mpopcnt -maes}"
...
printf "libcrux_crate_version=%s\n" "${LIBCRUX_CRATE_VERSION:-0.0.10}"
printf "libcrux_harness_cflags=%s\n" "${LIBCRUX_HARNESS_CFLAGS:--O3 -march=native}"
...
printf "botan_adapter_mode=internal-core-direct\n"
printf "botan_cache_policy=wire-key-parse-and-operation-rebuild-per-call\n"
printf "botan_modules=%s\n" "${BOTAN_MODULES:-ml_kem,keccak_perm_bmi2}"
printf "botan_cxx=%s\n" "${BOTAN_CXX:-<matching C_COMPILER>}"
printf "botan_cc_family=%s\n" "${BOTAN_CC_FAMILY:-<matching C_COMPILER>}"
printf "botan_compiler_pairing=required\n"
printf "botan_allow_compiler_fallback=%s\n" "${BOTAN_ALLOW_COMPILER_FALLBACK:-<unset>}"
...
printf "botan_harness_cflags=%s\n" "${BOTAN_HARNESS_CFLAGS:--O3 <profile ARCH_CFLAGS> -fomit-frame-pointer -fno-stack-protector -std=c11}"
printf "botan_harness_cxxflags=%s\n" "${BOTAN_HARNESS_CXXFLAGS:--O3 -march=native}"
